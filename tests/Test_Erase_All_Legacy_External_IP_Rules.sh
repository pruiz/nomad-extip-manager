...
# and delete the several legacy EXTERNAL_IP with and without signatures
...
iptables -w -t nat -I POSTROUTING -j MASQUERADE #< Adding some garbage...
iptables -w -t nat -I POSTROUTING -m comment --comment "NEIM:5678" -j EXTERNAL_IP 
iptables -w -t nat -I POSTROUTING -j EXTERNAL_IP 
